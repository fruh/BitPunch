...
int BPU_asn1EncodePriKey(char **buffer, int *size, const BPU_T_Mecs_Ctx *ctx) {
    ...
    *size = 0;
    ...
    rc = asn1_create_element(definitions, BPU_STR_ASN1_MECS_PRI_KEY_NAME, &asn1_element);
    if (rc != ASN1_SUCCESS) {
    ...
    if (ctx->type == BPU_EN_MECS_BASIC_GOPPA) {
        rc = asn1_write_value(asn1_element, "oid", BPU_STR_ASN1_OID_MECS_BASIC_GOPPA, strlen(BPU_STR_ASN1_OID_MECS_BASIC_GOPPA));
    }
    ...
    rc = asn1_write_value(asn1_element, "m", (void *)&(ctx->code_ctx->math_ctx->mod_deg), sizeof((ctx->code_ctx->math_ctx->mod_deg)));
    if (rc != ASN1_SUCCESS) {
        BPU_printError("asn1_write_value(): m %d", rc);
        return rc;
    }
    rc = asn1_write_value(asn1_element, "t", (void *)&(ctx->code_ctx->t), sizeof((ctx->code_ctx->t)));
    if (rc != ASN1_SUCCESS) {
        BPU_printError("asn1_write_value(): t %d", rc);
        return rc;
    }
    rc = asn1_write_value(asn1_element, "mod", (void *)&(ctx->code_ctx->math_ctx->mod), sizeof(ctx->code_ctx->math_ctx->mod));
    if (rc != ASN1_SUCCESS) {
        BPU_printError("asn1_write_value(): mod %d", rc);
        return rc;
    }
    rc = asn1_write_value(asn1_element, "g", (void *)(ctx->code_ctx->code_spec->goppa->g->coef), sizeof(ctx->code_ctx->code_spec->goppa->g->coef[0]) * (ctx->code_ctx->code_spec->goppa->g->deg + 1));
    if (rc != ASN1_SUCCESS) {
        BPU_printError("asn1_write_value(): g %d", rc);
        return rc;
    }
    rc = asn1_write_value(asn1_element, "p", (void *)(ctx->code_ctx->code_spec->goppa->permutation->elements), sizeof(ctx->code_ctx->code_spec->goppa->permutation->elements[0]) * ctx->code_ctx->code_spec->goppa->permutation->size);
    if (rc != ASN1_SUCCESS) {
    ...
    if (ctx->code_ctx->type == BPU_EN_CODE_GOPPA) {
        ...
        // prepare H matrix
        h_mat_bs = sizeof(BPU_T_GF2_16x) * (ctx->code_ctx->code_spec->goppa->h_mat->k * ctx->code_ctx->code_spec->goppa->h_mat->n);
        ...
        for (i = 0; i < ctx->code_ctx->code_spec->goppa->h_mat->k; i++) {
            memcpy(h_mat + j, ctx->code_ctx->code_spec->goppa->h_mat->elements[i], ctx->code_ctx->code_spec->goppa->h_mat->n * sizeof(BPU_T_GF2_16x));
...
int BPU_asn1DecodePriKey(BPU_T_Mecs_Ctx **ctx, const char *buffer, const int size) {
    ...
    j = 0;
    for (i = 0; i < (*ctx)->code_ctx->code_spec->goppa->h_mat->k; i++) {
        memcpy((*ctx)->code_ctx->code_spec->goppa->h_mat->elements[i], tmp_buf + j, (*ctx)->code_ctx->code_spec->goppa->h_mat->n * sizeof(BPU_T_GF2_16x));
        j += (*ctx)->code_ctx->code_spec->goppa->h_mat->n * sizeof(BPU_T_GF2_16x);
    }
    free(tmp_buf);
...
int BPU_asn1EncodePubKey(char **buffer, int *size, const BPU_T_Mecs_Ctx *ctx) {
    ...
    *size = 0;
    ...
    rc = asn1_create_element(definitions, BPU_STR_ASN1_MECS_PUB_KEY_NAME, &asn1_element);
    if (rc != ASN1_SUCCESS) {
        BPU_printError("asn1_create_element(): %d", rc);
        return rc;
    }
    ...
    if (ctx->code_ctx->type == BPU_EN_CODE_GOPPA) {
        g_mat_bs = sizeof(BPU_T_GF2) * (ctx->code_ctx->code_spec->goppa->g_mat->k * ctx->code_ctx->code_spec->goppa->g_mat->elements_in_row);
        ...
        for (i = 0; i < ctx->code_ctx->code_spec->goppa->g_mat->k; i++) {
            memcpy(g_mat + j, ctx->code_ctx->code_spec->goppa->g_mat->elements[i], ctx->code_ctx->code_spec->goppa->g_mat->elements_in_row * sizeof(BPU_T_GF2));
...
int BPU_asn1DecodePubKey(BPU_T_Mecs_Ctx **ctx, const char *buffer, const int size) {
    ...
    j = 0;
    tmp = (*ctx)->code_ctx->code_spec->goppa->g_mat->elements_in_row * ((*ctx)->code_ctx->code_spec->goppa->g_mat->element_bit_size / 8);
    for (i = 0; i < (*ctx)->code_ctx->code_spec->goppa->g_mat->k; i++) {
        memcpy((*ctx)->code_ctx->code_spec->goppa->g_mat->elements[i], tmp_buf + j, tmp);
        j += tmp;
    }
    free(tmp_buf);
...
int BPU_asn1LoadFileToBuffer(char **buffer, long *size, const char *file_name) {
    ...
    *buffer = (char *) malloc(*size);
    ...
    if (tmp != *size) {
        BPU_printError("can not read data from file, read only %ld B, instead of %ld B", tmp, *size);
        free(*buffer);
        fclose(fp);
...
int BPU_asn1LoadKeyPair(BPU_T_Mecs_Ctx **ctx, const char *pri_key_file, const char *pub_key_file) {
    if (BPU_asn1LoadPriKey(ctx, pri_key_file) || BPU_asn1LoadPubKey(ctx, pub_key_file)) {
...
int BPU_asn1SaveKeyPair(const BPU_T_Mecs_Ctx *ctx, const char *pri_key_file, const char *pub_key_file) {
    if (BPU_asn1SavePriKey(ctx, pri_key_file) || BPU_asn1SavePubKey(ctx, pub_key_file)) {
...
int BPU_asn1WriteBufferToFile(const char *buffer, const long size, const char *file_name) {
    ...
    if (tmp != size) {
        BPU_printError("can not write buffer to file, written %ld B, instead of %ld B", tmp, size);
        fclose(fp);
