...
int BPU_gf2MatMalloc (BPU_T_GF2_Matrix ** m, int rows, int cols) {
    ...
    *m = (BPU_T_GF2_Matrix *) calloc (sizeof (BPU_T_GF2_Matrix), 1);
    ...
    for (i = 0; i < (*m)->k; i++) {
        (*m)->elements[i] =
            (BPU_T_GF2 *) calloc (1,
                                  sizeof (BPU_T_GF2) * (*m)->elements_in_row);
...
int BPU_gf2PolyMalloc (BPU_T_GF2_Poly * p, int len) {
    ...
    // allocate elemtens
    p->elements =
        (BPU_T_GF2 *) calloc (1, sizeof (BPU_T_GF2) * p->array_length);
...
int BPU_gf2QcMatrixMalloc (BPU_T_GF2_QC_Matrix * v, int element_count,
                           int element_size, int isVertical,
                           int is_I_appended) {
    int err = 0;
...
int BPU_gf2MatPermute (BPU_T_GF2_Matrix * inout,
...
void BPU_gf2PolyMulMod (const BPU_T_GF2_Poly * a, const BPU_T_GF2_Poly * b,
                        BPU_T_GF2_Poly * c, const BPU_T_GF2_Poly * m,
                        int crop) {
    int i;
